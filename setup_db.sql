...
CREATE TABLE shipments (
    ...
    courier          VARCHAR(50)   NOT NULL DEFAULT 'CHAZKI',
    delivery_service VARCHAR(30)   NOT NULL DEFAULT 'REGULAR',
    ...
    CONSTRAINT chk_shipments_delivery_service CHECK (delivery_service IN ('REGULAR', 'EXPRESS', 'PROGRAMADO')),
    CONSTRAINT chk_shipments_cost   CHECK (shipping_cost >= 0)
);

COMMENT ON TABLE  shipments               IS 'Envíos asociados a pedidos';
COMMENT ON COLUMN shipments.courier       IS 'Empresa de transporte — por defecto CHAZKI';
COMMENT ON COLUMN shipments.tracking_code IS 'Código de seguimiento o ID de servicio de Chazki';
